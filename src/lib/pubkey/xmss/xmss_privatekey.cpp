...
class XMSS_PrivateKey_Internal {
   public:
      ...
      size_t reserve_unused_leaf_index() {
         const size_t idx =
            static_cast<size_t>((static_cast<std::atomic<uint64_t>&>(*recover_global_leaf_index())).fetch_add(1));
      ...
      size_t unused_leaf_index() const { return static_cast<size_t>(*recover_global_leaf_index()); }

      size_t remaining_signatures() const {
         return m_xmss_params.total_number_of_signatures() - static_cast<size_t>(*recover_global_leaf_index());
      }
